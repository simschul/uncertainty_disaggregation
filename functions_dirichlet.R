...
plot_dirichlet <- function(x, mu = NULL, u = NULL, type, ...) {
  ...
  if (type == 4) {
    data <- x %>% melt %>%
      as.data.table
    means <- data[, list(mean = mean(value)), by = Var2]
    return(ggplot(data, aes(x = value, fill = as.factor(Var2), col = as.factor(Var2))) +
      geom_density(alpha = 0.4) +
      geom_vline(data = means, aes(xintercept = mean, col = as.factor(Var2))))
...
rgamss <- function(n, shape, scale=1, do.log=FALSE) {

  a <- shape
  if(a > 0.2) o <- log(rgamma(n, a, 1 / scale)) else {
