...
get_child_proxies <- function(tree,
                                      emissions = 'emissions_CRF',
                                      sd = 'sd_NIR',
                                      sample = 'sample',
                                      ...) {
  tree$Do(function(node) {
    if (has_attribute_tree(node, sd) && isNotLeaf(node)) {
      leaves_without_sample <- !sapply(node$leaves, has_attribute_tree, sd)
      if (all(leaves_without_sample)) {
        leaves_path <- node$Get('pathString', filterFun = function(x) {
          isLeaf(x) & !has_attribute_tree(x, sd) & has_attribute_tree(x, emissions)
        })
        leaves_emissions <- node$Get(emissions, filterFun = function(x) {
          isLeaf(x) & !has_attribute_tree(x, sd) & has_attribute_tree(x, emissions)
        })
        ...
        node[['mapping_target']] <- leaves_path
        node[['proxy']] <- alphas
...
#' @param N
#' @param beta
#' @param dir_type one of dir1, dir_maxent, gdir
#'
#' @return
#' @export
#'
  ...
  tree$Do(function(node) {
    if (has_attribute_tree(node, sample) && isNotLeaf(node)) {
      ...
      if (all(leaves_without_sample)) {
        ...
        if (dir_type == 'dir1') {
          sample_children <- gtools::rdirichlet(N, rep(1, length(alphas)))

        } else if (dir_type == 'dir_maxent') {
          # find gamma using maxent
          tryCatch({
            res <- find_gamma_maxent2(shares = alphas,
                                      eval_f = eval_f)
        ...
        } else if (dir_type == 'gdir') {
            # extract betas from leaves
            betas <- node$Get(beta, filterFun = function(x) {
              isLeaf(x) & has_attribute_tree(x, beta)
            })
            if (!all(is.finite(betas))) {
              stop("Not all leaves have a beta value. ")
            }
            sample_children <- rgdir(N, alphas, betas)

        } else {
          stop('dir_type not implemented. must be one of "dir1", "dir_maxent", "gdir"')
        }
  ...
  tree$Do(function(node) {
    if (!has_attribute_tree(node, sd) # node does not have uncertainty info
      ...
      if (!has_attribute_tree(node, emissions)) {
        # empty node withouth emissions
        node[[sample]] <- rep(NA, N)
      } else if (node[[emissions]] == 0) {
        # node with zero emissions
        node[[sample]] <- rep(0, N)
      } else {
        # node with emissions: Choose between different default distributions
        if (dist_default == 'exp') {
          #exponential dist
          node[['dist']] <- 'exp'
          node[[sample]] <- rexp(N, rate = 1 / node[[emissions]])
        } else if (dist_default == 'truncnorm') {
          # truncanted normal
          ...
          stop('not implemented yet. dist_default must be one of exp or truncnorm')
        }
      }
    }
  }, filterFun = isLeaf)
