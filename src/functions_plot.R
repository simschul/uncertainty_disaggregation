...
# Fill
scale_fill_colorblind2 = function(.ColorList = c(1,8), ...){
  scale_fill_discrete(..., type = colorblind_pal()(8)[.ColorList])
}

# Color
scale_color_colorblind2 = function(.ColorList = c(1,8), ...){
  scale_color_discrete(..., type = colorblind_pal()(8)[.ColorList])
}
...
diag_hist_colour <- function(data, mapping,
                             palette = NULL,  # named vector, e.g. c(x = "red", y = "blue")
                             bins = 20, ...)  # any extra geom_histogram() args
{
  # which variable are we plotting?
  var_name <- rlang::as_name(mapping$x)

  # default palette = the standard ggplot hue palette, one colour per column
  if (is.null(palette)) {
    palette <- scales::hue_pal()(ncol(data))
    names(palette) <- names(data)
  }

  # pick the colour that belongs to this variable
  fill_col <- palette[[var_name]]

  # build the histogram
  ggplot(data = data, mapping = mapping) +
    geom_histogram(colour = fill_col,
                   fill   = fill_col,
                   bins   = bins, ...) +
    theme_bw()
}
...
fmt_scalar_for_title <- function(x, variable, digits = 2) {
  if (is.null(x)) {
    paste0(variable, " = NULL")
  } else {
    paste0(
      variable,
      " = ",
...
fmt_vec_for_title <- function(x, variable, digits = 2) {
  if (is.null(x)) {
    paste0(variable, " = NULL")
  } else {
    paste0(
