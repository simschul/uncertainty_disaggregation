...
ggplot(dt_gamma[type != 'multivariate' & case != 'no correlations'],
  ...
  # geom_vline(data = dt_gamma_summary[type != 'multivariate'
  #                                    & case != 'no correlations'] ,
  #            aes(xintercept = mean, col = type), linetype = 'dashed') +
  # geom_vline(data = dt_gamma_summary[type != 'multivariate'
  #                                    & case != 'no correlations'] ,
  #            aes(xintercept = q025, col = type), linetype = 'dashed') +
  # geom_vline(data = dt_gamma_summary[type != 'multivariate'
  #                                    & case != 'no correlations'] ,
  #            aes(xintercept = q975, col = type), linetype = 'dashed') +
  #
  #geom_density() +
...
# _c) Gamma dist without multivariate ======================================
dt_gamma <-  dt2[name %in% c('uv_gamma')]

dt_gamma <- dt_gamma[!(name == 'uv_gamma' & variable == 'sample')]
dt_gamma[variable == 'sample', type := 'original']
dt_gamma[variable == 'sample_new' & name == 'uv_gamma', type := 'neglecting\ncorrelations']
dt_gamma[type == 'original', alpha := 0.6]
dt_gamma[type != 'original', alpha := 0.3]
dt_gamma[, type := factor(type, levels = c('original', 'neglecting\ncorrelations'))]
